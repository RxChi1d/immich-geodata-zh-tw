...
def cmd_enhance(args):
    ...
    extra_file = args.extra_file or os.path.join("geoname_data", "extra_data", "TW.txt")
    output_file = args.output_file or os.path.join("output", "cities500_optimized.txt")
    enhance_data.update_cities500(cities_file, extra_file, output_file, args.min_population)
...
def cmd_locationiq(args):
    ...
    if not api_key:
        logger.critical("必須提供 LocationIQ API Key (透過 --locationiq-api-key 或環境變數 LOCATIONIQ_API_KEY)")
        sys.exit(1)
    ...
    for cc in args.country_code:
        ...
        generate_geodata_locationiq.process_file(cities_file, output_file, cc, batch_size)
...
def cmd_translate(args):
    ...
    alternate_name_file = args.alternate_name_file or os.path.join(output_folder, "alternate_chinese_name.csv")
    translate.translate_cities500(metadata_folder, cities500_file, output_file, alternate_name_file)
...
def cmd_release(args):
    ...
    if not args.pass_enhance:
        ...
        extra_file = os.path.join("geoname_data", "extra_data", "TW.txt")
        enhance_data.update_cities500(cities_file, extra_file, enhanced_output, 100)
    else:
    ...
    if not args.pass_locationiq:
        ...
        api_key = args.locationiq_api_key or os.environ.get("LOCATIONIQ_API_KEY")
        if not api_key:
            logger.critical("必須提供 LocationIQ API Key (透過 --locationiq-api-key 或環境變數)")
            sys.exit(1)
        qps = args.locationiq_qps if args.locationiq_qps is not None else int(os.environ.get("LOCATIONIQ_QPS", "1"))
        generate_geodata_locationiq.set_locationiq_config(api_key, qps)
        ...
        for cc in args.country_code:
            ...
            generate_geodata_locationiq.process_file(enhanced_output, locationiq_output, cc, args.batch_size)
    else:
    ...
    if not args.pass_translate:
        ...
        translate.translate_cities500(metadata_folder, enhanced_output, translated_cities, alternate_name_file)
        admin1_file = os.path.join(output_folder, "admin1CodesASCII_optimized.txt")
...
def main():
    ...
    parser_cleanup.add_argument("--output-folder", type=str, default="./output", help="輸出資料夾")
    parser_cleanup.set_defaults(func=cmd_cleanup)
    
    # prepare 子命令
    parser_prepare = subparsers.add_parser("prepare", help="下載並處理 geoname 資料")
    parser_prepare.add_argument("--country-code", type=str, nargs="+", default=["TW"],
                                help="國家代碼，可提供多個代碼，如: TW JP")
    parser_prepare.add_argument("--update", action="store_true", help="刪除現有資料並重新下載")
    parser_prepare.set_defaults(func=cmd_prepare)
    
    # enhance 子命令
    parser_enhance = subparsers.add_parser("enhance", help="優化 cities500 資料")
    parser_enhance.add_argument("--cities-file", type=str, help="原始 cities500.txt 路徑")
    parser_enhance.add_argument("--extra-file", type=str, help="額外城市資料檔案路徑")
    parser_enhance.add_argument("--output-file", type=str, help="輸出優化後的檔案路徑")
    parser_enhance.add_argument("--min-population", type=int, default=100, help="最小人口數")
    ...
    parser_locationiq.add_argument("--locationiq-qps", type=int, default=1, help="LocationIQ 每秒查詢次數限制")
    parser_locationiq.set_defaults(func=cmd_locationiq)
    
    # translate 子命令
    parser_translate = subparsers.add_parser("translate", help="翻譯地名資料")
    parser_translate.add_argument("--source-folder", type=str, default="./geoname_data", help="原始資料夾")
    parser_translate.add_argument("--output-folder", type=str, default="./output", help="輸出資料夾")
    parser_translate.add_argument("--alternate-name-file", type=str, help="替代名稱檔案路徑")
    parser_translate.set_defaults(func=cmd_translate)
    
    # modify 子命令
    parser_modify = subparsers.add_parser("modify", help="修改臺灣行政區代碼")
    parser_modify.add_argument("--data-folder", type=str, default="./geoname_data", help="原始資料夾")
    parser_modify.add_argument("--output-folder", type=str, default="./output", help="輸出資料夾")
    parser_modify.set_defaults(func=cmd_modify)
    
    # pack 子命令
    parser_pack = subparsers.add_parser("pack", help="產生 release 壓縮檔")
    parser_pack.add_argument("--output-folder", type=str, default="./output", help="輸出資料夾")
    ...
    parser_release.add_argument("--update-prepare", action="store_true", help="prepare 時重新下載資料")
    parser_release.add_argument("--data-folder", type=str, default="./geoname_data", help="原始資料夾")
    parser_release.add_argument("--output-folder", type=str, default="./output", help="輸出資料夾")
    parser_release.add_argument("--country-code", type=str, nargs="+", default=["TW"],
                                help="國家代碼，可提供多個代碼，如: TW JP")
    parser_release.add_argument("--overwrite", action="store_true", help="覆蓋已存在的資料")
    parser_release.add_argument("--batch-size", type=int, default=100, help="每批次寫入數量")
    parser_release.add_argument("--locationiq-api-key", type=str, help="LocationIQ API Key")
    parser_release.add_argument("--locationiq-qps", type=int, default=1, help="LocationIQ 每秒查詢次數限制")
    parser_release.add_argument("--pass-cleanup", action="store_true", help="跳過 cleanup 步驟")
    parser_release.add_argument("--pass-prepare", action="store_true", help="跳過前置處理")
    parser_release.add_argument("--pass-modify", action="store_true", help="跳過 modify 步驟")
    parser_release.add_argument("--pass-enhance", action="store_true", help="跳過 enhance 步驟")
    parser_release.add_argument("--pass-locationiq", action="store_true", help="跳過 locationiq 步驟")
    parser_release.add_argument("--pass-translate", action="store_true", help="跳過 translate 步驟")
    parser_release.add_argument("--pass-pack", action="store_true", help="跳過 pack 步驟")
