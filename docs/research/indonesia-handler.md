...
> **文件定位**：本文件為印尼 handler 實作**前**的研究與決策紀錄（snapshot），
> 彙整階段二（投影／座標）與階段三（Wikidata 語言／P31／P131／覆蓋率）兩份
> 正式實驗報告全文，記錄當時的評估數據與設計選項，內容反映成文時的狀態，不隨
> 後續實作同步更新。**現行處理邏輯請以
> [`docs/zh-tw/indonesia-admin-processing.md`](../zh-tw/indonesia-admin-processing.md)
> 為準**。
...
- **資料**：全精度 BIG 圖資（`geometryPrecision=6`，無幾何簡化）。
  - desa（村級）：可用 feature 83,461 筆（原始 83,462 筆，1 筆因缺
    `WADMPR`/`WADMKK` 過濾）。
  - kecamatan（郡級）：7,299 筆。
