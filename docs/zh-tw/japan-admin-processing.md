...
- **設計理由**：
  ...
  - **保留定位精度**：各區的獨立記錄與中心點仍保留在資料集中，僅顯示名稱統一為市名
- **範例**：
  - 神奈川県, 横浜市, 中区 → 顯示「横浜市」

> [!NOTE]
> 若需恢復「市＋區」顯示（例如「横浜市中区」），可在 `JapanGeoDataHandler` 類別中將 `SEIREI_SHI_CITY_NAME_ONLY` 常數設為 `False`。
...
| 行政區類型 | N03_001 | N03_003 | N03_004 | N03_005 | admin_2 顯示 | 說明 |
|-----------|---------|---------|---------|---------|-------------|------|
| 普通市 | 北海道 | （空） | 釧路市 | （空） | 釧路市 | 市不屬郡，直接顯示市名 |
| 直轄町村 | 東京都 | （空） | 小笠原村 | （空） | 小笠原村 | 離島由都直接管理 |
| 特別區 | 東京都 | （空） | 渋谷区 | （空） | 渋谷区 | 東京都 23 區 |
| 政令市區 | 神奈川県 | 横浜市 | 横浜市 | 中区 | 横浜市 | 僅顯示市名，保留各區記錄以維持定位精度 |
| 郡轄村（無同名） | 新潟県 | 岩船郡 | 関川村 | （空） | 関川村 | 簡潔模式 |
| 郡轄村（有同名） | 北海道 | 古宇郡 | 泊村 | （空） | 古宇郡泊村 | 多郡同名時自動補郡名 |
