...
| **Admin 2** | 市/區/郡 | 기초자치단체 | 252 個（譯名 231 種） | `sggnm` |
| **Admin 3** | 洞/邑/面 | 행정동/법정동 | 3,534 筆（名稱 2,802 種） | 解析自 `adm_nm` |
| **Admin 4** | 原始 Admin 3 | - | 僅市區拆分時（492 筆） | 保留原始資料用 |

> **統計基準**：以 `HangJeongDong_ver20260401` 圖資產生的 `meta_data/kr_geodata.csv`（共 3,558 筆）為準。
...
1. **特別市／廣域市／特別自治市**：統一輸出為「X市」
   - 範例：서울특별시 → 首爾市、부산광역시 → 釜山市、세종특별자치시 → 世宗市
...
|------|----------|------|----------|
| **Admin 1** | 內建對照表 + Wikidata QID | 17 | 17 個項目 |
| **Admin 2** | Wikidata 批次翻譯 | 228 | 228 個項目 |
...
2. **Admin 2**：使用 Wikidata 翻譯，數量適中（228 個）
3. **Admin 2（世宗市）**：使用手動對照表，確保 100% 繁體中文覆蓋率
4. **Admin 3**：保留韓文原文，避免 3,500+ 次 API 請求，且對使用者體驗影響較小

### Admin 1 翻譯方法

使用**內建對照表**提供臺灣慣用名稱（非 Wikidata 官方名稱），涵蓋全部 17 個廣域市／道。完整對照表位於 `src/pipeline/extract/handlers.rs`：
...
> [!NOTE]
> 第 4、5 步命中的英文或韓文標籤，會被南韓 handler 的 `is_valid_chinese_translation` 擋下並回退韓文原名，因此不會成為輸出。又因 Wikidata 條目幾乎必定有 en 標籤，第 6 步的「中文維基百科條目標題」在實務上不會被觸發。現行 `geoname_data/KR_wikidata_cache.json` 的 245 筆翻譯中，來源僅有 zh-TW（180）、OpenCC 轉繁（46）與 zh-Hant（19）三種。
...
> **例外**：고성군 的兩處譯名不同（江原道為高城郡、慶尚南道為固城郡），因此在繁體中文層並不衝突。

#### Admin 3 層級

約 229 個重複名稱（以 `HangJeongDong_ver20260401` 圖資統計），不影響 Immich 顯示（通常僅顯示至 Admin 2）。
