...
pub struct ProductionTranslateOptions {
    /// LocationIQ 逆地理查詢快取目錄（production 為 `cache/locationiq/`）。
...
/// `cache/locationiq/`，與 handler extract 產物 `meta_data/{cc}_geodata.csv`
/// 分屬不同目錄。後者由 enhance 階段（`admin1_load` / `cities500_load`）以
/// 明確檔名消費，其內容早已寫入 cities500，不應在此重複載入。
///
/// Reason: 目錄雖已分離，檔名守衛仍保留——它同時擋住誤放的檔案，並負責大小寫
/// 正規化（`--country-code us` 會產出 `us.csv`，而 cities500 國碼為大寫，
/// 不正規化會靜默失效）。歷史背景：兩種檔案曾共用 `meta_data/`，舊版直接把
/// 檔名 stem 當國碼，handler geodata 檔被當成國碼 `tw_geodata` 的 metadata
/// 載入——25 萬列永不命中、不影響輸出，也沒有任何 log。
