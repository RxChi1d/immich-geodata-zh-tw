...
本專案重新整理 Immich 使用的地理資料：臺灣、日本、南韓、泰國、印尼改用當地公開的行政區圖資重建地點，其餘地區則補上臺灣慣用的中文譯名。改用官方圖資會讓地點的密度大幅上升，因此發布前會再做一次剪枝，在不改變任何查詢結果的前提下刪掉多餘的點，讓資料庫更小、查詢更快。若使用 Docker Compose，安裝只需要在 `docker-compose.yml` 加一行設定。
...
- **發布前會剪枝**：官方圖資讓地點密度大幅上升，發布前會刪掉「刪了也不會改變任何查詢結果」的多餘點位。資料庫佔用減少約四分之一，密集地區的地點查詢快上約三倍，而顯示的地名完全不變。做法見[地名點剪枝](docs/zh-tw/point-pruning.md)。
...
發布前的剪枝不會讓這個情況變嚴重——被刪掉的點，都是經過驗證「刪了之後任何座標查出來的地名都不變」的多餘點位。

**地廣人稀的地區只顯示國名，沒有城市與行政區。**

這類地區本來就沒有足夠的地名資料：Immich 只在座標周圍 25 公里內尋找地點，找不到時就退回國界資料，只顯示國名。以加拿大為例，約 88% 的陸地在 25 公里內沒有任何收錄的地點。

這是上游地名資料的覆蓋範圍所致，要改善只能補充該地區的地點資料。
