...
本版本改用地點類別（而非人口門檻）判斷 GeoNames 收錄範圍，新增發布前剪枝大幅縮小資料庫，並調整馬來西亞、印尼的城市顯示層級；同時更新臺灣、印尼、南韓圖資，並修正多項自行建置與 nightly 建置的穩定性問題。

**升級提醒**：
- 本版本調整了全球地名收錄規則，並更新臺灣、印尼、南韓的行政區資料。請在更新後於 Immich 執行「重新擷取照片中繼資料」，以套用最新資料。

### Added
- **自動修正標錯的行政區**：使用 LocationIQ 查詢的地區，若 GeoNames 記載的一級行政區與 Natural Earth、LocationIQ 兩個來源一致指向另一個行政區，會自動更正，兩者不一致時保留原值；馬來西亞修正 95 個地點。自行建置會同步產生 `{國碼}_admin1_fixes.csv` 記錄本次判定。**建議重新提取中繼資料**。
- **自行建置可指定 LocationIQ 欄位設定路徑**：新增 `--locationiq-address-fields` 參數，修正在非專案目錄執行下載執行檔時找不到設定的問題。使用官方發布資料者不受影響。

### Changed
- **地名收錄改用地點類別判斷**：不再以登記人口達 100 人為門檻（許多真實聚落的人口欄位是空白或 0），改依聚落與行政區點類別收錄；廢棄、已毀與歷史地點不再收錄。馬來西亞收錄地名由 791 個增為 19,903 個，人煙稀少地區不再只顯示國名而沒有行政區。**建議重新提取中繼資料**。
- **發布前剪除多餘地名點**：新增剪枝步驟，刪除「刪了也不會改變任何查詢結果」的重複點位。全球地名減少 33.5%，密集地區的地點查詢延遲降低約三分之二，顯示的地名與行政區完全不變。**不需要重新提取中繼資料**。做法見[地名點剪枝](docs/zh-tw/point-pruning.md)。
- **馬來西亞城市名改為縣級**：固定顯示縣（daerah）名，取代原本混雜村、鎮、市轄區的英文地名；19,903 個地點中 96.5% 已有對應中文縣名。**建議重新提取中繼資料**。
- **印尼城市名改為郡級**：改顯示郡（kecamatan）取代原本過粗的縣市層級，可顯示的地點名稱由 514 個增為 6,908 個，並彙整國教院譯名表、維基資料與 OpenStreetMap 補上 286 個中文郡名。**建議重新提取中繼資料**。判準見[City 層級的選擇條件](docs/zh-tw/city-level-criteria.md)。
- **臺灣圖資更新**：村里界資料更新至國土測繪中心 2026-06-24 版，新增數個村里並修正邊界。
- **印尼圖資更新**：行政區邊界更新至地理空間資訊局 2026-06-12 版。**建議重新提取中繼資料**。
- **南韓行政區改制**：資料更新至 2026-07-01 生效的最新行政區劃，全羅南道與光州廣域市合併為「全南光州市」，仁川中區、東區、西區改制為濟物浦區、永宗區、西海區與黔丹區。**建議重新提取中繼資料**。
- **南韓地名改用韓國官方漢字**：與日本地名的處理方式一致，改以韓國官方漢字表記為準（如「淸州市」「尙州市」），共 147 筆用字調整。
- **文件重整**：README 聚焦安裝流程，其餘說明移至 `docs/`，並新增文件索引、貢獻指南與安全性回報方式，補充 macOS 加速器的重啟與分離式部署設定。
- **資料來源聲明**：`NOTICE.md` 補齊 Natural Earth、Wikidata、中文維基百科、韓文維基百科與 i18n-iso-countries 的授權聲明。
- **自行建置目錄結構調整**：LocationIQ 查詢結果、官方圖資中繼檔與外部 vendored 資料分別移至 `data/locationiq/`、`data/handler/`、`data/vendor/`，依產生方式分類。使用官方發布資料者不受影響。
- **自行建置記憶體用量降低**：讀取 Natural Earth 行政區邊界改為直接解析所需欄位，記憶體峰值由 384 MB 降為 73 MB。

### Removed
- **自行建置移除 `--min-population` 參數**：收錄條件不再與人口有關，該參數已無作用。使用官方發布資料者不受影響。

### Fixed
- **Nightly 版本恢復穩定更新**：修正兩個造成每週自動建置中斷或跳過發布的問題——查詢速率未依服務的每分鐘上限節流、以及發布條件誤判為中繼資料未變動。現在額度用完時會保留已查結果並照常發布。
- **單一查不到的座標中斷整次建置**：逆地理查詢對查無此地的座標，原本視為錯誤中止整次建置且不保存進度，現在改為跳過並於下次重試。僅影響自行建置且指定 LocationIQ 國家的情況。
- **同座標地名整組消失**：多筆地名共用完全相同座標時，原本的保留條件可能同時落空使整組被丟棄；現在固定保留編號最小的一筆，50 個原本消失的地名因此復原。
- **自行建置的 LocationIQ 譯名品質**：查詢結果原本會覆蓋既有中文城市名（例如「蕉賴」變成「吉隆坡」），中文字元也會被存成亂碼；現在只在缺中文名時才採用查詢結果，並修正編碼問題。
- **南韓地名錯誤**：修正冠岳區、松坡區、驪州市、全南光州市咸平郡等 4 處長期存在的錯誤地名，共影響 69 筆。
- **安裝文件與提示修正**：更正手動部署下載目錄、指定版本網址與 `docker-compose.yml` 服務名稱等錯誤說明；安裝位置有誤時一併提示可用 `IMMICH_BUILD_DATA` 指定 geodata 目錄。
- **自行建置不再強制要求 LocationIQ API key**：所有地區改用官方圖資後，只有指定需要 LocationIQ 的國家才會要求金鑰。
- **自行建置的翻譯階段不再讀取無用資料**：修正誤將各國官方圖資中繼檔當成 LocationIQ 查詢結果載入的問題，減少約 25 萬列無用資料的讀取。

### Security
- **相依套件更新**：`quinn-proto` 更新至 0.11.17，解除 QUIC 封包重組可造成記憶體耗盡的警示。本專案未啟用 HTTP/3，未受影響。
