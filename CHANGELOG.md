...
### Added
- **自動修正標錯的行政區**：使用 LocationIQ 查詢的地區，若 GeoNames 記載的一級行政區（州、省）有誤，現在會自動更正。馬來西亞共修正 95 個地點，例如八打靈縣與安邦原本歸在吉隆坡（實際屬雪蘭莪州）、居林縣與瓜拉姆達縣原本歸在檳城州（實際屬吉打州）。判定需要 Natural Earth 行政區邊界與 LocationIQ 兩個來源指向同一個行政區才會更動，兩者不一致時保留原值。**建議重新提取中繼資料**，既有照片才會顯示更正後的行政區。
- **行政區修正紀錄**：自行建置時，每次執行都會在 `data/locationiq/` 產生 `{國碼}_admin1_fixes.csv`，記錄本次判定的每一筆行政區分歧，含採納與未採納及其原因，供逐週比對。使用官方發布資料者不受影響。
- **自行建置可指定 LocationIQ 欄位設定路徑**：新增 `--locationiq-address-fields` 參數指定 `address_fields.json` 的位置。原本只認專案目錄下的相對路徑，在其他目錄執行下載的執行檔會找不到設定而無法啟動。使用官方發布資料者不受影響。
- **馬來西亞地名中文化**：馬來西亞的城市名改用當地通用的華文地名，例如八打靈再也、民丹莪、萬里茂，共 453 個原本顯示英文名的城市改為中文。**建議重新提取中繼資料**，既有照片才會顯示新地名。

### Changed
- **馬來西亞城市名改為縣級**：馬來西亞的城市名原本混雜村、鎮、市轄區等不同層級，同一個縣內最多出現 9 種不同名字，且多為英文村名。現在固定取縣（daerah）名，19,903 個地點中 96.5% 有對應的中文縣名，涵蓋全國 144 個縣中的 143 個；同一個縣內名字一致的比例由 30% 提升至 79%。少數 OSM 未標示縣的地點維持顯示原有地名，與先前相同。**建議重新提取中繼資料**，既有照片才會顯示新的城市名。
- **印尼城市名改為郡級**：印尼的城市名原本顯示縣／市（kabupaten），平均一個名字涵蓋 3,705 平方公里、53.6 萬人，比臺灣的鄉鎮市區粗 37 倍——在烏布拍的照片顯示「巴釐省・吉亞尼亞爾縣」、婆羅浮屠顯示「中爪哇省・馬格朗縣」，都不是一般人用來指稱該地的名字。現在改顯示郡（kecamatan），可顯示的地點名稱由 514 個增為 6,907 個。郡級的中文譯名不像縣市級那樣普遍存在，彙整國教院譯名表、維基資料與 OpenStreetMap 三個來源後共補上 286 個郡名，其餘顯示官方印尼文；省名仍為中文。補上的部分集中在會拍照的地方——雅加達 44 個郡中有 39 個、峇里島 57 個中有 15 個、西加里曼丹 173 個中有 43 個顯示中文（烏布、庫塔、金塔馬尼、婆羅佛塔皆在其中）。未採用查無第二來源佐證的機器音譯，寧可顯示印尼文原名。地點變細後剪枝能刪除的重複點位同時減少，全球地名總數增加約 17%。**建議重新提取中繼資料**，既有照片才會顯示新的城市名。判準見[City 層級的選擇條件](docs/zh-tw/city-level-criteria.md)。
...
- **原始碼結構調整**：五個超過專案 500 行上限的模組依關注點拆分（翻譯流程、CLI、LocationIQ、Wikidata 查詢、打包輸出），測試改與被測程式碼同檔。發布內容與行為完全不變，已逐行核對搬移前後的程式碼與 289 個測試名稱皆相同。使用官方發布資料者不受影響。
- **自行建置記憶體用量降低**：讀取 Natural Earth 行政區邊界時不再先展開成通用 JSON 結構，改為直接解析所需欄位。該階段的記憶體峰值由 384 MB 降為 73 MB、常駐用量由 340 MB 降為 35 MB，載入時間由 227 ms 降為 63 ms。發布內容與現行版本完全相同，已以真實圖資驗證解析結果逐位元一致。使用官方發布資料者不受影響。
...
### Fixed
- **單一查不到的座標中斷整次建置**：逆地理查詢服務對無法定位的座標（外海、無定義區域）回報「查無此地」時，原本被當成流程錯誤而中止整次建置，且該次已查到的結果不會被保存。現在改為跳過該座標並繼續，下次建置會再試一次。僅影響自行建置且指定了 LocationIQ 國家的情況。
