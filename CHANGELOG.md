...
### Changed
- **地名收錄改用地點類別判斷**：不再以登記人口達 100 人為門檻（許多真實聚落的人口欄位是空白或 0），改依聚落與行政區點類別收錄；廢棄、已毀與歷史地點不再收錄。行政區正確率明顯提升，例如臺灣由 69.52% 提升至 95.34%。**建議重新提取中繼資料**。
...
- **印尼城市名改為郡級**：改顯示郡（kecamatan）取代原本過粗的縣市層級，可顯示的地點名稱由 514 個增為 6,907 個，並彙整國教院譯名表、維基資料與 OpenStreetMap 補上 286 個中文郡名。**建議重新提取中繼資料**。判準見[City 層級的選擇條件](docs/zh-tw/city-level-criteria.md)。
