...
- **LocationIQ 查詢快取改放獨立目錄**：自行建置時，LocationIQ 的逆地理查詢結果改寫入 `cache/locationiq/`，不再與各地區官方圖資的中繼資料共用 `meta_data/`。兩者欄位相同但性質相反——前者可安全刪除後重新查詢，後者不可任意重建——分開存放後不必再從檔名判斷哪些檔案可以清理。使用官方發布資料者不受影響。
