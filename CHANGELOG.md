...
### Added
- **自動修正標錯的行政區**：使用 LocationIQ 查詢的地區，若 GeoNames 記載的一級行政區（州、省）有誤，現在會自動更正。馬來西亞共修正 8 個城市，例如實達百林原本歸在雪蘭莪州（實際屬吉隆坡）、白沙羅升鎮與再也鎮原本歸在吉隆坡（實際屬雪蘭莪州）。判定需要 Natural Earth 行政區邊界與 LocationIQ 兩個來源指向同一個行政區才會更動，兩者不一致時保留原值。**建議重新提取中繼資料**，既有照片才會顯示更正後的行政區。
- **行政區修正紀錄**：自行建置時，每次執行都會在 `data/locationiq/` 產生 `{國碼}_admin1_fixes.csv`，記錄本次判定的每一筆行政區分歧，含採納與未採納及其原因，供逐週比對。使用官方發布資料者不受影響。
- **馬來西亞地名中文化**：馬來西亞的城市名改用當地通用的華文地名，例如八打靈再也、民丹莪、萬里茂，共 453 個原本顯示英文名的城市改為中文。**建議重新提取中繼資料**，既有照片才會顯示新地名。

### Changed
- **發布前剪除多餘地名點**：改用官方圖資後地點密度大幅上升，發布前新增一道剪枝，刪除「刪了也不會改變任何查詢結果」的多餘點位。全球地名由 487,372 個減為 324,296 個（−33.5%），解壓後的地理資料由 63.9 MB 降為 47.8 MB，匯入後的資料庫由 221 MB 降為 164 MB，日本、印尼、臺灣、南韓、泰國等密集地區的地點查詢延遲降低約三分之二。顯示的地名與行政區完全不變——每個被刪的點都經過驗證，635 萬個測試座標查出的結果零差異，不同的行政區名稱一個都沒有消失。**不需要重新提取中繼資料**，既有照片的地點顯示不受影響。做法見[地名點剪枝](docs/zh-tw/point-pruning.md)。
- **地名收錄改用地點類別判斷**：GeoNames 的地名是否收錄，改依地點類別（聚落與行政區點）判斷，不再要求登記人口達 100 人。大量真實聚落的人口欄位是空白或 0（沒人填，不是沒人住），以人口篩選會連同它們一起丟掉。以官方圖資比對行政區正確率，改用類別判斷後印尼由 85.65% 提升至 97.56%、臺灣由 69.52% 提升至 95.34%。馬來西亞收錄的地名由 791 個增為 19,903 個；人煙稀少的地區改為納入行政區點，不再出現只顯示國名、沒有行政區的情況。廢棄、已毀與歷史地點則不再收錄。**建議重新提取中繼資料**，既有照片才會套用新的地名與行政區。
- **自行建置移除 `--min-population` 參數**：收錄條件不再與人口有關，該參數已無作用而移除。使用官方發布資料者不受影響。
...
### Fixed
- **每週自動建置因查詢額度中斷**：每週的自動建置會在逆地理查詢階段中止，nightly 版本因此停止更新。查詢速率原本只依「每秒上限」設定，未考慮服務同時存在的「每分鐘上限」，實際速率超出後即被拒絕；且查詢額度用完時整個流程視為失敗，當次已查到的結果不會保存，下次得重新查詢。現在速率改以較嚴格的每分鐘上限為準，額度用完時自動建置會保留已查結果並照常發布，剩餘地點留待下次補查。
- **同座標地名整組消失**：多個地名共用完全相同的座標時，原本要求同一筆同時是人口最多且編號最小才保留，兩個條件落在不同筆時整組都會被丟掉，該座標因此完全查不到地名。現在同座標固定保留編號最小的一筆，全球資料中有 50 個原本消失的地名因此復原。
- **自行建置的 LocationIQ 譯名覆蓋既有地名**：指定 LocationIQ 國家時，查詢結果會蓋掉 GeoNames 既有的中文城市名，把城市名換成上一層的轄區名，例如「蕉賴」變成「吉隆坡」。現在只在既有資料沒有中文名時才採用查詢結果。臺灣、日本、南韓、泰國與印尼已改用官方圖資，不經此流程，官方發布資料不受影響。
