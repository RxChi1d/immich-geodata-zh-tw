...
### Added
- **非容器部署支援**：`update_data.sh` 可在 macOS 原生 worker、LXC 與裸機等非容器環境安裝資料，安裝路徑改為自動偵測目錄版面。需要指定時可用 `IMMICH_SERVER_ROOT` 覆寫，一旦設定即為唯一搜尋範圍，指定錯誤會直接失敗而非改裝到其他位置；geodata 位置則沿用 Immich 自己的 `IMMICH_BUILD_DATA`。
- **安裝結果驗證**：安裝後比對下載內容與模組解析結果，確認資料確實寫入 Immich 會載入的套件，不一致時直接報錯，不再出現「安裝成功但資料沒生效」的情況。驗證涵蓋整份語系資料，包含實際決定國名顯示的 `en.json`。找得到 `node` 時交由 `node` 解析，找不到時以相同的模組解析規則自行判斷，因此非容器部署也一樣會驗證。
- **失敗自動復原**：所有檢查都在修改系統檔案之前完成；安裝過程中任何一步失敗，都會將 geodata 與語系資料還原為安裝前的狀態，不再留下只完成一半的更新。
- **`--print-paths` 參數**：安裝前可先確認腳本解析出的 geodata 與 i18n-iso-countries 目標路徑。
- **`--archive` 參數**：直接以本機既有的 `release.tar.gz` 安裝，適用於離線環境或重複安裝同一份資料。

### Changed
- **安裝腳本不再依賴 Node.js**：改以目錄結構判斷 i18n-iso-countries 位置，取代原本讀取 `package.json` 版本再推斷路徑的做法。若 Immich 再次變更目錄結構，腳本會掃描並找出套件的新位置，不需要再為新的版本分界點修改腳本；同時偵測到多個位置時會提出警告，找不到已安裝的套件時直接失敗。
- **腳本主流程包在 `main()` 中**：整合式部署的 `entrypoint` 以 `bash <(curl ...)` 邊下載邊執行，下載中斷時原本會執行已收到的部分並以成功狀態結束，留下只做一半的安裝。改為最後一行才呼叫主流程，檔案沒收完就不會執行任何步驟；開場訊息刻意保留在主流程之外，讓中斷在日誌中留下可察覺的落差。
- **Release 頁面精簡**：發佈頁面不再附帶未壓縮 CLI binary、ldd 診斷清單與 build manifest 等工程性檔案；預編譯 CLI 仍以 tar.gz 與對應 sha256 校驗檔提供。

### Fixed
- **非 root 環境安裝失敗**：安裝時僅在以 root 執行的情況下調整檔案擁有者，避免在 macOS 等環境因 `chown` 失敗而中斷，導致 geodata 已安裝但 i18n-iso-countries 未安裝的半完成狀態。
- **安裝失敗不再靜默**：來源資料缺漏時以非零狀態結束，並在下載前先驗證安裝路徑；下載遇到 HTTP 錯誤時直接失敗，不再把錯誤頁面當成壓縮檔。
- **備份失效**：i18n-iso-countries 為符號連結（pnpm 版面）時，改為備份其下的 `langs` 目錄，而非再產生一條指向同一份檔案的連結。
- **地理資料下載穩定性**：放寬大型資料檔的下載逾時，避免上游伺服器速率下降時，200 MB 以上的資料檔因逾時過短而下載失敗。

### Security
- **語系檔覆寫可能寫穿符號連結**：`langs` 下的檔案若為符號連結，原本的覆寫方式會寫入連結指向的檔案；以 root 執行安裝時，這讓可寫入該目錄的使用者得以影響其他位置的檔案。改為逐檔寫入隨機命名的暫存檔後 rename，同時避免改動 pnpm store 中可能被硬連結共用的檔案，也不再刪除本專案未提供的上游語系檔。
